...
import { avaterPlaceholder } from "@/constants";
...
const Sidebar = ({fullname,avatar,email}:Props) => {
  ...
  return (<aside className="sidebar">
      ...
      <ul className="flex flex-1 flex-col gap-6">
        {navItems.map(({url,name,icon})=>(
          <Link key={name} href={url} className="lg:w-full">
            <li className={cn("sidebar-nav-items",
    ...
    <div className="sidebar-user-info">
      <Image src={avatar} alt ="avatar" width={44} height={54} className="sidebar-user-avatar"/>
      <div className="hidden lg:block">
        <p className="caption">{fullname}</p>
        <p className="caption">{fullname}{email}</p>
        </div>
    </div>
    </aside>
  )
